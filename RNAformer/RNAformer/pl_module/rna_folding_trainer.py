...
class RNAFoldingTrainer(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model = RNAFormer(cfg_model)
    ...
    def training_step(self, batch, batch_idx):
        ...
        logits_mat, mask = self.model(batch['src_seq'], batch['length'], batch['pdb_sample'], n_cycles=max_cycle)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        with torch.no_grad():
            ...
            self.log(
                f"val/{self.data_module.valid_sets[dataloader_idx]}/loss",
                loss,
                on_step=False,
                on_epoch=True,
    ...
    def on_validation_epoch_end(self):
        ...
        for dataset_idx, dataset_name in enumerate(self.data_module.valid_sets):
    ...
    def configure_optimizers(self):
        ...
        if self.cfg_train.optimizer.cpr:
            parameters = group_parameters_for_cpr_optimizer(self.model)

            optimizer = AdamCPR(parameters, lr=self.cfg_train.optimizer.lr, betas=self.cfg_train.optimizer.betas,
                                kappa_init_param=self.cfg_train.cpr_config.kappa_init_param,
                                kappa_init_method=self.cfg_train.cpr_config.kappa_init_method)
