...
class AxialAttention(nn.Module):
    ...
    def forward(self, pair_act, pair_mask, cycle_infer=False):
        ...
        if self.use_flash_attn:
            ...
            # if self.training and self.checkpointing and not cycle_infer:
            #     output_unpad = torch.utils.checkpoint.checkpoint(flash_attn_varlen_qkvpacked_func,
            #                                                      x_unpad, cu_seqlens, max_s, 0.0,
            #                                                      self.softmax_scale, False, False)
            # else:
            output_unpad = flash_attn_varlen_qkvpacked_func(
                x_unpad, cu_seqlens, max_s, 0.0,
            ...
            # if self.training and self.checkpointing and not cycle_infer:
            #     output = torch.utils.checkpoint.checkpoint(self._multihead_attn,
            #                                                query, key, value, pair_mask,
            #                                                 self.mask_bias, self.softmax_scale, batch_size,
            #                                                 seqlen, self.num_head, self.head_dim)
            # else:
