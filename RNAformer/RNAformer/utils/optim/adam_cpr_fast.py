...
class AdamCPR(Optimizer):
    def __init__(self,
        ...
        if self.kappa_init_method not in ['warm_start', 'uniform', 'dependent']:
...
if __name__ == "__main__":
    ...
    parameters = group_parameters_for_optimizer(model)

    optimizer = AdamCPR(parameters, lr=1e-3, kappa_init_param=1000, kappa_init_method='warm_start', reg_function='l2')

    outout = model(torch.ones(10))
    loss = outout.sum()
    loss.backward()
    optimizer.step()

    for param_group in optimizer.param_groups:
        if param_group['weight_decay'] == 1.0:
            for name, param in model.named_parameters():
                ...
                try:
                    ...
                    print(state['lagmul'])
                    print(state['kappa'])
                    print(state['kappa_update'])
                    print(state['step'])
                    print(state.keys())
                except:
                    pass

        # fabric.log("optim/weight_decay", param_group['weight_decay'], optim_step_count)
