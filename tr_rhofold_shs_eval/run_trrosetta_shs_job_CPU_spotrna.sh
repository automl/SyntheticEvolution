#!/bin/bash
#SBATCH --job-name=trrosetta_shs_from_spotrna
#SBATCH --partition=cpu-single 
#SBATCH --output=logs/trrosetta_shs_from_spotrna_%j.out
#SBATCH --error=logs/trrosetta_shs_from_spotrna_%j.err
...
OUT_DIR="${OUT_DIR:-${BASE_DIR}/predictions_trrosetta_on_shs_from_spotrna_folder}"
...
python "$PIPELINE_SCRIPT" \
  ...
  --n-decoys 5 \
  --skip-existing \
  --log-file "logs/trrosetta_shs_from_spotrna_${SLURM_JOB_ID}.log"
