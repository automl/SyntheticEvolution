...
#SBATCH --partition=gpu-single          # shared GPU partition on Helix (1 node max)
#SBATCH --output=logs/trrosettarna_%j.out
#SBATCH --error=logs/trrosettarna_%j.err
#SBATCH --time=01:00:00                 # geometry step is fast; Rosetta folding is slow
#SBATCH --export=NONE                   # recommended on Helix for reproducibility

# ── Resources ─────────────────────────────────────────────────────────────────
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16             # rMSA MSA search benefits from many CPUs
#SBATCH --mem=64gb
#SBATCH --gres=gpu:A40:1              # GPU needed for geometry prediction step
                                        # alternatives: gpu:A40:1, gpu:H200:1
...
CMD=(
    ...
    --gpu             0                  # GPU 0; use -1 for CPU-only; Determines which GPU is used for geometry prediction step. 
    --n_cpu           "$((SLURM_CPUS_PER_TASK / 2))"
    --n_decoys        5
