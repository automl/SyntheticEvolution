#!/bin/bash
#SBATCH --job-name=trrosetta_shs_from_rnafold
#SBATCH --partition=cpu-single 
#SBATCH --output=logs/trrosetta_shs_from_rnafold_%j.out
#SBATCH --error=logs/trrosetta_shs_from_rnafold_%j.err
...
PREP_DIR="${PREP_DIR:-${BASE_DIR}/prepared_shs_inputs_from_rnafold_folder}"
OUT_DIR="${OUT_DIR:-${BASE_DIR}/predictions_trrosetta_on_shs_from_rnafold_folder}"
...
python "$PIPELINE_SCRIPT" \
  ...
  --n-decoys 5 \
  --skip-existing \
  --log-file "logs/trrosetta_shs_from_rnafold_${SLURM_JOB_ID}.log"
