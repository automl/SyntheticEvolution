    ...
    for s in data['sequences']:
        for k, v in s.items():
            ...
            sample = {
                    ...
                    'pdb_id': name[:4],
                    'json_id': name,
                    'msa_length': int((len(str(s[k]['unpairedMsa']).split('\n')) - 1) / 2),
            }
...
if __name__ == '__main__':
    ...
    #     print(row)
    json_dir = "/Users/Iris/Desktop/missing_msa_files_iris"
    results = main(json_dir)

    # print(results)
    print(results.groupby(['has_msa', 'mol_type']).count())

    results.to_csv("/Users/Iris/Desktop/msa_info_nonx_data_af3_preds_for_iris.csv", index=False)  #  sep="\t", index=False)
