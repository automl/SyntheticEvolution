...
class BPRNAFeatureExtractor:
    """Extract RNA secondary structure features using bpRNA"""
    
    def __init__(self, sequence: str, structure: str):
        self.sequence = sequence
        self.structure = structure
        
    def extract_bprna_motifs(self):
        """Run bpRNA and extract motif information"""
        # Create temporary directory and files
        os.makedirs('./tmp_data', exist_ok=True)
        date = datetime.now().strftime("%Y_%m_%d-%I%M%f")
        tmp_file = f'./tmp_data/rna_{date}'
        
        # Write sequence and structure to dbn file
        with open(f'{tmp_file}.dbn', 'w') as f:
        ...
        # Parse results
        motifs = self._parse_bprna_output(f'{tmp_file}.st')
        
        # Cleanup
        os.remove(f'{tmp_file}.dbn')
        os.remove(f'{tmp_file}.st')
        
        return motifs
    
    def _parse_bprna_output(self, output_file):
        """Parse bpRNA output file to extract motifs"""
        with open(output_file) as f:
            lines = f.readlines()
            
        # Find start of motif section
        start_idx = 0
        for i, line in enumerate(lines):
        ...
        for line in lines[start_idx:]:
            if not line.strip():
                continue
            parts = line.split()
            motif_type = re.sub(r"[\W\d_]+$", "", parts[0])
            motif_id = re.findall(r"[\W\d_]+$", parts[0])[0]
            info = parts[1:]
            motifs.append({
                'type': motif_type.lower(),
                'motif_id': motif_id,
                'info': info
            })
...
def compare_with_dssr(dssr_features, bprna_features):
    """Compare DSSR and bpRNA feature counts"""
    comparison = {
        'stems': {
            'dssr': len(dssr_features['stems']),
            'bprna': len(bprna_features[bprna_features['type'] == 's'])
        },
        'hairpins': {
            'dssr': sum(1 for h in dssr_features['hairpin_loops'] if h['length'] > 0),
            'bprna': len(bprna_features[bprna_features['type'] == 'h'])
        },
        'internal_loops': {
            'dssr': len(dssr_features['internal_loops']),
            'bprna': len(bprna_features[bprna_features['type'] == 'i'])
        },
        'multibranch': {
            'dssr': sum(1 for m in dssr_features['multibranch_loops'] if m['degree'] > 0),
            'bprna': len(bprna_features[bprna_features['type'] == 'm'])
        }
    }
    return comparison
...
if __name__ == "__main__":
    # Example usage
    sequence = "GGCUAGAUAGCUCAGCGGGAGAGCAUUUGCAUGGCAUGCAAGAGGUCAGCGGUUCGAUCCCGCUUAUCUAGCCACCA"
    structure = "(((((((..((((.......))))((((((((...))))))))....(((((.......))))))))))))....."
    
    # Extract features using bpRNA
    bprna = BPRNAFeatureExtractor(sequence, structure)
    bprna_features = bprna.extract_bprna_motifs()
    
    # Extract features using DSSR (from our previous implementation)
    from utils.secondary_features import analyse_secondary_structure
    dssr_features = analyse_secondary_structure("path/to/dssr.json")
    
    # Compare results
    comparison = compare_with_dssr(dssr_features, bprna_features)
    print("Feature comparison between DSSR and bpRNA:")
    for feature, counts in comparison.items():
        print(f"{feature}:")
        print(f"  DSSR: {counts['dssr']}")
        print(f"  bpRNA: {counts['bprna']}")
