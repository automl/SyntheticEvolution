...
# nucleotide_atoms = {
#     "A": {
#         "base": ["N1", "C2", "N3", "C4", "C5", "C6", "N6", "N7", "C8", "N9"],
#         "sugar": ["C1'", "C2'", "C3'", "C4'", "C5'", "O2'", "O3'", "O4'", "O5'"],
#         "phosphate": ["P", "OP1", "OP2"]
#     },
#     "G": {
#         "base": ["N1", "C2", "N2", "N3", "C4", "C5", "C6", "O6", "N7", "C8", "N9"],
#         "sugar": ["C1'", "C2'", "C3'", "C4'", "C5'", "O2'", "O3'", "O4'", "O5'"],
#         "phosphate": ["P", "OP1", "OP2"]
#     },
#     "C": {
...
#     },
#     "T": {
#         "base": ["N1", "C2", "O2", "N3", "C4", "O4", "C5", "C7", "C6"],
...
def get_resi_from_polynucleotide_chain(polynucleotide_chain): # Get full resi list to avoid motif: A(-3), U(-2), U(-1), U(0)...
    resi_set = set()
...
def list_interactions_from_cif(file_path, list_name):
    ...
    # peptide_chains = [strand for strand in strand_ids if strand not in polynucleotide_chains]
    ...
    #     is_protein = True
    #     if 'polyribonucleotide' in entity_types:
    #         is_rna = True
    #         rna_chain_id = get_longest_chain_id_from_db(file_path, is_rna)
    #         # peptide_chain = find_closest_protein_chain(file_path, rna_chain_id, peptide_chains)
    #         polynucleotide_chains = rna_chain_id
    #         # print("closest_protein_chain_id", peptide_chain)
    #     if 'polydeoxyribonucleotide' in entity_types:
    #         is_rna = False
    #         dna_chain_id = get_longest_chain_id_from_db(file_path, is_rna)
    #         # peptide_chain = find_closest_protein_chain(file_path, dna_chain_id, peptide_chains)
    #         polynucleotide_chains = dna_chain_id
    ...
    # print("final_chain_pairs:", chain_pairs)
    # peptide_chain = calculate_highest_contacts(chain_pairs)
    ...
    #     for peptide_chain in peptide_chains:
        ...
        # def extract_numeric_part(residue):
        #     return int(''.join(filter(str.isdigit, residue)))
        # try:
        #     sorted_combined_interactions = sorted(combined_interactions.values(),
        #                                           key=lambda x: (int(x[0].resi), int(x[1].resi)))
        # except ValueError: # 7mdl.cif: I don't know why only in this case cmd resi gives residue index 47B instead of 47.
        #     sorted_combined_interactions = sorted(
        #         combined_interactions.values(),
        #         key=lambda x: (extract_numeric_part(x[0].resi), extract_numeric_part(x[1].resi))
        #     )
        # print("sorted_combined_interactions", sorted_combined_interactions)
            ...
            with open(output_file, 'w') as f:
                ...
                # if (entity_types[strand_ids.index(polynucleotide_chain)] == 'polyribonucleotide'):
                    ...
                    if (find_entity_type(peptide_chain, strand_ids, entity_types) != 'polypeptide(L)'):
                        ...
                        polynucleotide_chainA = get_resi_from_polynucleotide_chain(polynucleotide_chain)
                        ...
                        if chain_id == None:
                            f.write("No multiple interactions found.\n")
                            for interaction in sorted_combined_interactions:
                                ...
                                f.write(f"\n Interaction type: {interaction_type}. Chain1: {chain1_info.chain}/{chain1_info.resn}"
                                        f"{chain1_info.resi} ({chain1_class})."
                                        f" Chain2: {chain2_info.chain}/{chain2_info.resn}{chain2_info.resi} {chain2_class}"
                        ...
                        # print("!!!full_rna_sequence", full_rna_sequence)
                        # print("rna_motif_indices", rna_motif_indices)
                        # print("rna_motif_visual", rna_motif_visual)
                ...
                # elif (entity_types[strand_ids.index(polynucleotide_chain)] == 'polydeoxyribonucleotide'):
                    ...
                    # print("full_na_sequence", full_na_sequence)
                    ...
                    # print("full_dna_sequence", full_dna_sequence)
                ...
                # print("interface_rna_atom_ids", interface_rna_atom_ids)
                # print(contact_list)
...
def adjust_motif_sequence(full_sequence_list, motif_sequence):
    # Find the offset: difference between where full_sequence_list starts and 1
    # offset = int(full_sequence_list[0]) - 1
    # adjusted_motif_sequence = [motif - offset for motif in motif_sequence]
...
def extract_rna_motif(sorted_combined_interactions, polynucleotide_chain, file_path):
    ...
    # rna_motif_visual = ''.join([resn for resn, resi in sorted_rna_motif])
...
def extract_base_to_base_nucleotides(sorted_combined_interactions):
    ...
    # print(base_to_base_interactions)
...
def extract_dna_motif(sorted_combined_interactions, polynucleotide_chain, file_path):
    ...
    # print("dna_motif_set", dna_motif_set)
    sorted_dna_motif = sorted(dna_motif_set, key=lambda x: int(x[1]))
    dna_motif_indices = [int(resi) for resn, resi in sorted_dna_motif]
    # print("dna_motif_indices", dna_motif_indices)
    # print("polynucleotide_chain", polynucleotide_chain)
    ...
    if len(file) in [4, 6] and not all(index in polynucleotide_chain for index in map(str, dna_motif_indices)):
        ...
        sorted_dna_motif = sorted(dna_motif_set, key=lambda x: int(x[1]))
        dna_motif_indices = [int(resi) for resn, resi in sorted_dna_motif]
        # print("after dna_motif_indices", dna_motif_indices)
    ...
    if first_chain_index != 1:
        ...
        # print(f"Adjusted dna_motif_indices: {dna_motif_indices}")
        updated_dna_motif_set = set()
        for i, (nucleotide, _) in enumerate(sorted_dna_motif):
            updated_dna_motif_set.add((nucleotide, dna_motif_indices[i]))
        dna_motif_set = updated_dna_motif_set
...
def extract_aa_motif(sorted_combined_interactions):
    ...
    for interaction in sorted_combined_interactions:
        ...
        if len(chain1_info.resn) > 1:
            aa_motif_set.add((chain1_info.resn, chain1_info.resi))
        elif len(chain2_info.resn) > 1:
    ...
    # print("aa_motif", aa_motif_string)
...
def calculate_aac(amino_acid_motif):
    ...
    aac = f"[{', '.join(aac_list)}]"
    # print("aac", aac)
    # aac_list = [aac.get(aa, 0) for aa in amino_acids] # List of frequencies in the order of the amino acids
    # print(round(sum(aac_list), 6))
...
def extract_atom_id(sorted_combined_interactions):
    ...
    #     atom_ids.append((chain1_info.id, chain2_info.id))
    # return atom_ids
    atom_ids = set()
    for interaction in sorted_combined_interactions:
        chain1_info, chain2_info, _, _, _, _, chain1_id, chain2_id = interaction
...
def extract_rna_atom_id(sorted_combined_interactions, polynucleotide_chains):
    # print("polynucleotide_chains", polynucleotide_chains)
    atom_ids = set()
    for interaction in sorted_combined_interactions:
        chain1_info, chain2_info, _, _, _, _, chain1_id, chain2_id = interaction
...
def highlight_sequence(full_rna_string, indices):
    ...
    # Problem fixed with get_resi_from_polynucleotide_chain()
    # def find_and_align_motif(full_rna_string, motif):
    #     # Align the 1st motif with the RNA sequence and return the adjusted start index if incorrect.
    #     # motif = rna_motif_visual.replace("|", "")
    #     motif_found = False
    #     # Search for the motif in the nucleotide string
    #     for i in range(len(full_rna_string) - len(motif) + 1):
    #         if ''.join(full_rna_string[i:i + len(motif)]) == motif:
    #             motif_found = True
    #             return i + 1  # 1-based index of where the motif starts
    #     if not motif_found:
    #         return None
    # def adjust_indices(indices, motif_start_index, original_start_index):
    #     # Adjust indices to align motif with the sequence
    #     offset = original_start_index - motif_start_index
    #     return [i - offset for i in indices if i - offset > 0]

    rna_nucleotides = handle_modified_nucleotides(full_rna_string)
    # motifs = rna_motif_visual.split('|')
    # first_motif = motifs[0]
    # second_motif = motifs[1] if len(motifs) > 1 else None
    # if len(first_motif) == 1 and not second_motif:
    #     motif_start_index = indices[0] if indices else 1 # Skip if the first motif is only 1 nucleotide
    # elif len(first_motif) == 1 and second_motif:
    #     # Only align starting from the second motif
    #     original_start_index = indices[1] if len(indices) > 1 else 1
    #     second_motif_start_index = find_and_align_motif(full_rna_string, second_motif)
    #     if second_motif_start_index == None: # No alignment found if motif length less than 3
    #         highlighted_sequence = full_rna_string
    #         return highlighted_sequence
    #     # Adjust indices only for those starting from the second motif
    #     elif second_motif_start_index != original_start_index:
    #         second_motif_indices = indices[1:]  # Indices starting from the second motif
    #         adjusted_indices = adjust_indices(second_motif_indices, second_motif_start_index, original_start_index)
    #         # Merge the first motif index back into the adjusted list
    #         indices = [indices[0]] + adjusted_indices
    # else:
    #     # Find the start index of the motif in the actual RNA sequence
    #     original_start_index = indices[0] if indices else 1
    #     motif_start_index = find_and_align_motif(full_rna_string, rna_motif_visual)
    #     if motif_start_index == None:
    #         highlighted_sequence = full_rna_string
    #         return highlighted_sequence
    #     # Adjust indices if they don't align with the motif position
    #     elif motif_start_index != original_start_index:
    #         indices = adjust_indices(indices, motif_start_index, original_start_index)
...
def update_exp_pdb_with_motifs(file_path, rna_motif, length_rna_motif, dna_motif, aa_motif, contact_list, rna_prot_interface_atom_ids,
    ...
    if len(file_name) > 6:
        if is_protein and is_rna and not is_dna:
            # print(rna_motif, aa_motif, contact_list, rna_prot_interface_atom_ids,
            #                                           interface_rna_atom_ids, base_name)
