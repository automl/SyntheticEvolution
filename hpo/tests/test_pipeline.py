...
import run_shs_dssr_pipeline as pipeline
from dssr import parse_pairs
from scoring import score_pairs, normalize_pairs, aggregate

FIXTURE = json.loads(Path(__file__).with_name('dssr_example.json').read_text())
SEQ = 'AGUAGUAGUAGUAGA'


class Tests(unittest.TestCase):
    def test_actual_dssr(self):
    ...
    def test_resume_and_failure_no_partial_loss(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            pipeline.write_json(directory/'trial.json',dict(fingerprint='abc',parameters={}))
            pipeline.write_json(directory/'job.json',dict(job_id='7'))
            model = directory/'rna_00000/af3/model.cif'
            model.parent.mkdir(parents=True)
            model.write_text('mock model')
            pipeline.write_json(model.parent/'success.json',dict(model=str(model)))
            with patch.object(pipeline,'wait_job') as wait, patch.object(pipeline,'run_dssr',return_value=parse_pairs(FIXTURE,SEQ)):
            ...
            with self.assertRaises(ValueError): pipeline.run_pipeline({'N':3},cfg,rows,directory,'abc')
    ...
    def test_fresh_submission_and_ambiguous_guard(self):
        with tempfile.TemporaryDirectory() as tmp:
            directory = Path(tmp)
            cfg = dict(fixed={}, shs_python=sys.executable, generator_dir='SHS-Generator',
            ...
            def submit(args):
                ...
                task=json.loads((directory/'manifest.json').read_text())[0]
                model=Path(task['output'])/'rna_00000/rna_00000_model.cif'
                model.parent.mkdir(parents=True)
                model.write_text('mock model')
                pipeline.write_json(Path(task['output'])/'success.json',dict(model=str(model)))
                return '17'
            with patch.object(pipeline.subprocess,'run',side_effect=generate), patch.object(pipeline,'command',side_effect=submit) as submit_mock, patch.object(pipeline,'wait_job'), patch.object(pipeline,'run_dssr',return_value={(0,14)}):
    ...
    def test_folder_inputs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(len(pipeline.dataset(folder)),2)
            (folder/'duplicate.csv').write_text('id,sequence,pairs\nrna0,ACGU,[]\n')
            with self.assertRaises(ValueError): pipeline.dataset(folder)
